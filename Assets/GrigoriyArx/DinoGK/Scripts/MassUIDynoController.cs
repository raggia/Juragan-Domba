...
public class MassUIDynoController : MonoBehaviour
{
    ...
    private void SwitchAnimation(int targetState, string targetBlendShape)
    {
        ...
        *///---------------------------------------------------------------------
            ...
            if ((targetState == 99))// && (!waterDino))
            {
                TempIslands.SetActive(false);
                waterDino = true;

            }
            if ((targetState == 98))// && (waterDino))
            {
                TempIslands.SetActive(true);
                waterDino = false;
            }

            if (targetState == 10)
            {
                GlobalLand.SetActive(false);
                GlobalWater.SetActive(true);
                TempIslands.SetActive(false);
            }
            else if ((targetState != 10)&&(!waterDino))
            {
                GlobalLand.SetActive(true);
                GlobalWater.SetActive(false);
                TempIslands.SetActive(true);
            }            
            else if ((targetState != 10)&&(waterDino))
            {
